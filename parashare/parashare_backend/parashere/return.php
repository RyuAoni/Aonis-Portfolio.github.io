...
$user_lat   = $body['latitude'] ?? null;
$user_lon   = $body['longitude'] ?? null;

if (!$qr_address || $id_user === null || $user_lat === null || $user_lon === null) {
...
try {
    ...
    $closest_storage_name = '不明な傘立て';
    $min_distance = PHP_FLOAT_MAX;

    foreach ($storages as $storage) {
        $distance = getDistance(
            (float)$user_lat, (float)$user_lon,
            (float)$storage['adress_latitude'], (float)$storage['adress_longitude']
        );
        if ($distance < $min_distance) {
            $min_distance = $distance;
            $closest_storage_id = (int)$storage['id_storage'];
            $closest_storage_name = $storage['storage_name'];
        }
    }

    $threshold = 0.1; // 約20m
    if ($closest_storage_id === null || $min_distance > $threshold) {
        respond(409, ['ok' => false, 'error' => '傘立ての近くにいません。']);
    }
	...
	$sql = "UPDATE umbrella
        　  SET point = COALESCE(point, 0) + 7
            WHERE id_umbrella = :id_umbrella";
	$upUmbPoint = $dbh->prepare($sql);
	$upUmbPoint->execute([':id_umbrella' => $id_umbrella]);
